...
class Guardian(Base):
    ...
    # Multi-child support: track currently active patient
...
class Hospital(Base):
    """Hospital node for Thronos MEDICE network."""
    __tablename__ = "hospitals"
    id                    = Column(Integer, primary_key=True)
    name                  = Column(String)
    country               = Column(String)  # e.g. "GR", "DE", "AT"
    contact_email         = Column(String, unique=True)
    contact_phone         = Column(String, nullable=True)
    api_key               = Column(String, unique=True)
    # Optional: hospital can receive real-time alerts
    alert_webhook_url     = Column(String, nullable=True)
    alert_webhook_secret  = Column(String, nullable=True)
    # Alert triggers: what conditions should trigger webhooks
    alert_spo2_threshold  = Column(Integer, default=90)   # SpO2 < this %
    alert_hr_min          = Column(Integer, default=60)   # HR < this bpm
    alert_hr_max          = Column(Integer, default=130)  # HR > this bpm
